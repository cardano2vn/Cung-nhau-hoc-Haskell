...
instance Semigroup Config where
    Config{port=p1, host=h1} <> Config{port=p2, host=h2} =
        Config {port = if p2 /= defaultPort then p2 else p1,
                host = if h2 /= defaultHost then h2 else h1}

instance Monoid Config where
    mempty = Config {port = defaultPort, host = defaultHost}
...
instance Semigroup Emergency where
  (Emergency s1 d1 r1) <> (Emergency s2 d2 r2) = Emergency (s1 <> s2) (d1 <> d2) (r1 <> r2)

instance Monoid Emergency where
  mempty = Emergency mempty mempty mempty
